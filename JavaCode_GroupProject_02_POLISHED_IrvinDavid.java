...
class ColtsStaffDirectory extends JFrame {
    private final DefaultTableModel model;
    private final JTable table;
    private final JTextField searchField;
    private final List<Object[]> staffData = new ArrayList<>();
    ...
    private void filter(TableRowSorter<DefaultTableModel> sorter) {
        ...
        for (Object[] row : staffData) {
            if (isDepartmentHeader(row)) {
                String department = valueOf(row[2]);
                if (department.toLowerCase().contains(searchText)) {
            ...
            String name = valueOf(row[0]);
            String position = valueOf(row[1]);
            String department = valueOf(row[2]);

            if (name.toLowerCase().contains(searchText)
                    || position.toLowerCase().contains(searchText)
    ...
    private void populateTable(List<Object[]> rows) {
        ...
        for (Object[] row : rows) {
            if (isDepartmentHeader(row)) {
                model.addRow(row);
                currentDepartment = valueOf(row[2]);
            } else {
                if (!currentDepartment.equals(valueOf(row[2])) && !currentDepartment.isEmpty()) {
                    model.addRow(new Object[]{"", "", valueOf(row[2])});
                    currentDepartment = valueOf(row[2]);
                }
                model.addRow(row);
            }
    ...
    private boolean isDepartmentHeader(Object[] row) {
        return row != null && row.length >= 3 && valueOf(row[0]).isEmpty() && valueOf(row[1]).isEmpty() && !valueOf(row[2]).isEmpty();
    }
    ...
    private void initializeStaffSlots() {
        ...
        for (Object[] staffMember : staffMembers) {
            String department = staffMember[2].toString();
            if (!department.equals(currentDepartment)) {
                staffData.add(new Object[]{"", "", department});
                currentDepartment = department;
            }
            staffData.add(staffMember);
        }
        populateTable(staffData);
    }
